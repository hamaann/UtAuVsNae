...
func init() {
	load.InitalPro[load.InRooms] = func(gx, gy float64) {
		...
		RoomEvent[roomNum] = func(mx, my *float64, mdir *int, speed float64, room *int) {
			...
			event.TalkingBox(mx, my, mdir, speed, 0, 130-35, 130+35, 50-40, 50+10, 30, nil)
		}
		...
		RoomEvent[roomNum] = func(mx, my *float64, mdir *int, speed float64, room *int) {
			...
			teruteruTelling := event.TalkingBox(mx, my, mdir, speed, 1, -100-35, -100+35, 0-40, 0+10, 30, func() { goingEnc = true })
			if teruteruTelling {
